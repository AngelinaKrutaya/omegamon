...
def setup_module(module):
    utils.set_security(hostname, username, password, rtes[rte]['rte_hlq'], rte, omegamon, 'passphrase-external',
...
def check_xmls_command(d):
    d('m', key=keys.ENTER)
    assert d.find('ZDISP')
    d('b', key=keys.ENTER)
    assert d.find('ZPEEKBB')
    command = d.find('>                        ALLOCATED')
    command('-XMLS UUU,1EB0,10', key=keys.ENTER)
    assert d.find('Security check failed (INTERNAL)')
    d(key=keys.HOME)
    d('/pwd', key=keys.ENTER)
    d('ibm3', key=keys.ENTER)
    d(key=keys.ENTER)
    assert d.find('Job not found')



def test_classic_command_authorized(classic_in_out):
...
@pytest.mark.parametrize('classic_in_out', [(username[:-1]+'b' if username.upper().endswith('A') else username+'b')],
                         indirect=True)
def test_classic_command_non_authorized(classic_in_out):
    check_peek_panel(classic_in_out, 'Security System has determined that you are not authorized')


def test_classic_xmls_command_user1(classic_in_out):
    check_xmls_command(classic_in_out)


@pytest.mark.parametrize('classic_in_out', [(username[:-1]+'b' if username.upper().endswith('A') else username+'b')],
                         indirect=True)
def test_classic_xmls_command_user2(classic_in_out):
    check_xmls_command(classic_in_out)
